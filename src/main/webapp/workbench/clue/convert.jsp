...
<script type="text/javascript">
	...
	//	微转化按钮绑定事件
		...
		//	提交请求到后台，执行线索转换的操作，发出传统请求，请求结束后最终回到线索页
		//	根据“为客户创建交易”的复选框有没有勾来判断是否创建交易
			if ($("#isCreateTransaction").prop("checked")){
			    // alert("需要创建交易");
				//如果需要创建交易，除了有clueId之外还有其他信息
				//以下传参方式很麻烦
                <%--window.location.href = "workbench/clue/convert.do?clueId=${param.id}&money=&expectedDate=&name=&stage=&activityId="--%>
				<%--使用交易表单（name属性），提交表单post--%>
				<%--提交表单--%>
				$("#tranForm").submit();
			} else {
			    // alert("不需要创建交易");
