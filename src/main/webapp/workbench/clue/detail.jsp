...
<script type="text/javascript">
	...
	//	为关联活动的模态窗口搜索框绑定事件
		$("#aname").keydown(function (event) {
			...
			//    展现完列表后，记得将模态窗口磨人的回车行为禁用：回车后自动刷新
				return false;
			}
        })
            ...
            $.ajax({
                ...
                success:function (data) {
                    if (data.success){
                        ...
                        html += '<font color="gray">市场活动</font> <font color="gray">-</font> <b>${c.fullname}</b> <small style="color: gray;" id="s'+ data.cr.id +'"> '+ (data.cr.createTime) +' 由'+ (data.cr.createBy) +'</small>';
                        html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
                        html += '<a class="myHref" href="javascript:void(0);" onclick="editRemark(\''+ data.cr.id +'\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #63bbe6;"></span></a>';
                        html += '&nbsp;&nbsp;&nbsp;&nbsp;';
                        //动态生成的必须套在字符串中 错误写法：'+ n.id +'
                        html += '<a class="myHref" href="javascript:void(0);" onclick="deleteRemark(\''+ data.cr.id +'\')"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #e64743;"></span></a>';
                        html += '</div>';
                        html += '</div>';
                        html += '</div>';
                        $("#remark").before(html);
